...
class HiSumModule(LightningModule):
    ...
    def step(self, batch):
        ...
        x = batch
        y = batch['gtscore']
        scene_score, shot_score = self.forward(x['features'], x['scene_boundary'], x['shot_boundary'], x['mask']) # x['scene_mask'], x['shot_mask']
        ...
        return scene_score, shot_score, scene_loss, shot_loss, y
    
    def training_step(self, batch, batch_idx):
        torch.autograd.set_detect_anomaly(True)
        _, shot_score, scene_loss, shot_loss, targets = self.step(batch)
        
        loss = scene_loss + shot_loss
        # pdb.set_trace()
        #self.log("loss/train", loss, on_step=True, on_epoch=True, prog_bar=True, logger=False)
        self.logger.experiment.add_scalar("loss/train", loss, self.current_epoch) 
        # pudb.set_trace()
        mode = 'train'
        train_f1 = self.gen_sum_and_eval(mode, batch, shot_score)
        
        
        return {"loss": loss, "logits" : shot_score, "f1score": train_f1, "targets": targets, "scene_loss": scene_loss, "shot_loss": shot_loss}
    ...
    def training_epoch_end(self, outputs):
        ...
        print(f'\n@{self.current_epoch} scene_loss: {mean_scene_loss}, shot_loss: {mean_shot_loss}')
        

    def validation_step(self, batch, batch_idx):
        _, shot_score, sc_loss, sh_loss, targets = self.step(batch)
        mode = 'val'
        if self.exp_type == 'original':
            val_f1 = self.gen_sum_and_eval(mode, batch, shot_score)
        
        # val_f1 = sum(val_f1) / len(val_f1)

        loss = sc_loss + sh_loss

        return {"loss": loss, "logits" : shot_score, "f1score": val_f1, "targets": targets, "sc_loss": sc_loss, "sh_loss": sh_loss}
    ...
    def test_step(self, batch, batch_idx):
        ...
        scene_score, shot_score, _, _, _, _ = self.step(batch)
        
        self.save_vis(self.vis_dir, scene_score, shot_score, scene_boundary, shot_boundary, name=video_name[0])

        # I added this line
        _, shot_score, sc_loss, sh_loss, targets = self.step(batch)
        mode = 'test'
        if self.exp_type == 'original':
            test_f1 = self.gen_sum_and_eval(mode, batch, shot_score)
        ...
        return {"loss": loss, "logits" : shot_score, "f1score": test_f1, "targets": targets}
    ...
    def xavier_init(model):
        for name, param in model.named_parameters():
            if name.endswith(".bias"):
                ...
                bound = torch.sqrt(torch.tensor(6)) / torch.sqrt(torch.tensor(param.shape[0] + param.shape[1]))
    ...
    def save_vis(self, dir, scene_score, shot_score, scene_boundary, shot_boundary, name=None):
        if name is None:
